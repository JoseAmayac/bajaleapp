...
export function useMeals(date: string) {
  ...
  const getSignedUrls = async (rows: DailyMeal[]) => {
    ...
    if (!withImages.length)
      return rows.map((m) => ({ ...m, signed_url: undefined }));
    ...
    return rows.map((m) => {
      ...
      return {
        ...m,
        signed_url:
          idx >= 0 ? (signed[idx].data?.signedUrl ?? undefined) : undefined,
      };
    });
  };
  ...
  const updateMeal = async (
    id: string,
    updates: Partial<Pick<DailyMeal, "meal_type" | "description">>,
  ) => {
    const { data } = await supabase
      .from("daily_meals")
      .update(updates)
  ...
  const totalCalories = meals.reduce(
    (sum, m) => sum + (m.calories_estimated ?? 0),
    0,
  );
